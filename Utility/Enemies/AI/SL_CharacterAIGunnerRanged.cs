...
{
	public class SL_CharacterAIGunnerRanged : SL_CharacterAI
	{
        [XmlIgnore]
        [NonSerialized]
		...
		protected override void ApplyToCharacter(Character character)
		{
            ...
            AddBulletsRenewal(AISCombatRanged, CharacterAI);

			CharacterAI.GetAIStates();
		}

		public void AddBulletsRenewal(AISCombat combatState, CharacterAI characterAI)
		{
            AICInventoryNeedsItem bulletsEmptyCondition = new GameObject($"Condition_Inventory_Needs_Item").AddComponent<AICInventoryNeedsItem>();
			bulletsEmptyCondition.SearchingItemId = 4400080; //bullets
			bulletsEmptyCondition.NeededItemAmount = 1;

            bulletsEmptyCondition.DetectionUpdateTime = 1;
            bulletsEmptyCondition.transform.SetParent(combatState.transform);
            bulletsEmptyCondition.SubCondition = false;

            AIESpawnBullets effectSpawnBullets = new GameObject($"ValidEffects").AddComponent<AIESpawnBullets>();
            effectSpawnBullets.transform.SetParent(bulletsEmptyCondition.transform);

            bulletsEmptyCondition.GroupValidEffectTrans = bulletsEmptyCondition.transform;
		}

		public float AIContagionRange = 50f;

		public Vector2 Range = new Vector2(35, 60);
		...
		public float Suspicious_Range = 60f;

		public float Suspicious_TurnModif = 0.9f;

		public Vector2 Combat_ChargeTime = new Vector2(0.2f, 0.5f);
		...
		public Vector2 Combat_ChargeStartRangeMult = new Vector2(1.8f, 3f);
