...
{
	public class SL_CharacterAIMelee_RegenaratingInventoryItems : SL_CharacterAI
	{
		public int ItemIdToRegenerate = 6600070;
		public int RegenerationQuantity = 1;

		protected override void ApplyToCharacter(Character character)
		{
			AIRoot airoot = new GameObject("BasicMelee_AIRoot").AddComponent<AIRoot>();
			...
			AISCombatMelee aiscombatMelee = new GameObject("4_Combat").AddComponent<AISCombatMelee>();
			aiscombatMelee.transform.parent = airoot.transform;
			...
			aieswitchState3.ToState = aiscombatMelee;
			...
			aieswitchState6.ToState = aiscombatMelee;
			...
			aiscombatMelee.ChargeTime = this.Combat_ChargeTime;
			aiscombatMelee.TargetVulnerableChargeTimeMult = this.Combat_TargetVulnerableChargeModifier;
			aiscombatMelee.ChargeAttackRangeMult = this.Combat_ChargeAttackRangeMulti;
			aiscombatMelee.ChargeAttackTimeToAttack = this.Combat_ChargeTimeToAttack;
			aiscombatMelee.ChargeStartRangeMult = this.Combat_ChargeStartRangeMult;
			aiscombatMelee.AttackPatterns = this.Combat_AttackPatterns;
			aiscombatMelee.SpeedModifs = this.Combat_SpeedModifiers;
			aiscombatMelee.ChanceToAttack = this.Combat_ChanceToAttack;
			aiscombatMelee.KnowsUnblockable = this.Combat_KnowsUnblockable;
			aiscombatMelee.DodgeCooldown = this.Combat_DodgeCooldown;
			aiscombatMelee.CanBlock = this.CanBlock;
			aiscombatMelee.CanDodge = this.CanDodge;
			AICEnemyDetection aicenemyDetection4 = new GameObject("Detection").AddComponent<AICEnemyDetection>();
			aicenemyDetection4.transform.parent = aiscombatMelee.transform;
			...
			aieswitchState7.transform.parent = aiscombatMelee.transform;
			...
			CharacterAI characterAI = character.gameObject.AddComponent<CharacterAI>();
			characterAI.m_character = character;
			CharacterHelpers.FixCharacterAINullOnQuestEvent(characterAI);
            AddItemRegenaration(aiscombatMelee, characterAI);
			characterAI.AIStatesPrefab = airoot;
			characterAI.GetAIStates();
		}

		public void AddItemRegenaration(AISCombat combatState, CharacterAI characterAI)
		{
            AICInventoryNeedsItem inventoryNeedsItemCondition = new GameObject($"Condition_Inventory_Needs_Item").AddComponent<AICInventoryNeedsItem>();
			inventoryNeedsItemCondition.SearchingItemId = ItemIdToRegenerate;
			inventoryNeedsItemCondition.NeededItemAmount = RegenerationQuantity;

            inventoryNeedsItemCondition.DetectionUpdateTime = 1;
            inventoryNeedsItemCondition.transform.SetParent(combatState.transform);
            inventoryNeedsItemCondition.SubCondition = false;

            AIESpawnItemInInventory effectSpawnItems = new GameObject($"ValidEffects").AddComponent<AIESpawnItemInInventory>();
            effectSpawnItems.transform.SetParent(inventoryNeedsItemCondition.transform);

            inventoryNeedsItemCondition.GroupValidEffectTrans = inventoryNeedsItemCondition.transform;
		}
		...
		public Vector2 Combat_ChargeTime = new Vector2(4f, 8f);
